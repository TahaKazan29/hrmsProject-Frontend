...
import { getPostsPassive } from '../../../store/actions/postActions';
import { postsActive } from '../../../store/initialValues/post';

export default function PostPassive() {

    const dispatch = useDispatch();
    const {postsPassive} = useSelector(state => state.posts);  

    useEffect(() =>{
      dispatch(getPostsPassive())
    },[])

    const postlist = postsPassive.length ? (
      postsPassive.map((post) => (
        <tr key={post.id}>
