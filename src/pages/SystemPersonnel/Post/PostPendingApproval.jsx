...
import { getPostsWaitingApproval } from '../../../store/actions/postActions';

export default function PostPendingApproval() {

  const {postsWaitingApproval} = useSelector(state => state.posts)
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getPostsWaitingApproval());
  },[])
  const postList = postsWaitingApproval.length ? (
    postsWaitingApproval.map((post) => (
      <tr key={post.id}>
