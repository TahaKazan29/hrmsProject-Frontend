...
import { getPostsExpired } from '../../../store/actions/postActions';
import { useDispatch, useSelector } from 'react-redux';

export default function PostExpired() {

  const dispatch = useDispatch();
    const {postsExpired} = useSelector(state => state.posts);  

    useEffect(() =>{
      dispatch(getPostsExpired())
    },[])

    const postlist = postsExpired.length ? (
      postsExpired.map((post) => (
        <tr key={post.id}>
